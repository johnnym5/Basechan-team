...
export function ClockControl({ userProfile, permissions, systemConfig, className }: ClockControlProps) {
  ...
  const attendanceQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'attendance'), where('userId', '==', userProfile.id), where('date', '==', today), where('status', 'in', ['PENDING', 'APPROVED']), limit(1));
  ...
  useEffect(() => {
    ...
    if (isClockedIn && !isOnBreak && attendanceRecord?.clockIn) {
      timer = setInterval(() => {
        const now = new Date();
        const workedSeconds = differenceInSeconds(now, new Date(attendanceRecord.clockIn)) - (attendanceRecord.totalBreak || 0) - (attendanceRecord.idleTime || 0);
    ...
    } else { setShiftDuration('00:00:00'); setProgress(0); }
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      toast({ title: 'Shift Started', description: 'Pending HR approval.' });
  ...
  const handleToggleBreak = async () => {
      ...
      try {
          ...
          toast({ title: isOnBreak ? 'Break Ended' : 'Break Started' });
  ...
  const handleClockOut = async () => {
     ...
     try {
       ...
       toast({ title: 'Shift Ended', description: 'Session logged.' });
     } catch (e: any) { toast({ variant: 'destructive', title: 'Error', description: e.message }); }
     finally { setIsSubmitting(false); }
  };
  ...
  return (
    <section className={cn("card-bg rounded-2xl p-8 flex flex-col items-center justify-center text-center shadow-lg relative overflow-hidden", className)}>
      ...
      <h3 className="text-5xl font-bold mb-4 font-headline tracking-tighter">{isClockedIn ? (isOnBreak ? 'REST' : shiftDuration) : '00:00:00'}</h3>
      {isClockedIn && (
          <div className="w-full max-w-[200px] mb-8 space-y-1.5">
             <div className="flex justify-between text-[0.625rem] font-bold text-muted-foreground uppercase tracking-widest"><span>Progress</span><span>{Math.round(progress)}%</span></div>
      ...
      <div className={cn("w-full space-y-4 mb-8", !isClockedIn && "mt-4")}>
        {isClockedIn ? (
            <div className="grid grid-cols-2 gap-4">
                <Button variant={isOnBreak ? 'default' : 'outline'} className={cn("py-8 rounded-xl text-lg font-bold uppercase", isOnBreak ? "bg-emerald-600" : "border-amber-500/50 text-amber-500")} onClick={handleToggleBreak} disabled={isSubmitting}>
                ...
                <Button className="py-8 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-lg font-bold uppercase" onClick={handleClockOut} disabled={isSubmitting}>
                    {isSubmitting ? <Loader2 className="animate-spin" /> : <><LogOut className="mr-2 h-6 w-6" /> End</>}
                </Button>
            </div>
        ) : (
            <Button className="w-full py-8 bg-green-500 hover:bg-green-600 text-white rounded-xl text-2xl font-bold uppercase shadow-xl" onClick={handleClockIn} disabled={isSubmitting}>
      ...
      <div className="flex flex-col items-center gap-4 w-full pt-4 border-t border-white/5">
        <div className="flex items-center justify-center space-x-8">
            <div onClick={() => !isClockedIn && setLocation('OFFICE')} className={cn("flex flex-col items-center gap-2 cursor-pointer transition-all", location === 'OFFICE' ? "text-primary scale-110" : "text-gray-500 opacity-50")}>
                <Building className="w-6 h-6" /><span className="text-[0.625rem] font-bold uppercase">Office</span>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div onClick={() => !isClockedIn && setLocation('REMOTE')} className={cn("flex flex-col items-center gap-2 cursor-pointer transition-all", location === 'REMOTE' ? "text-primary scale-110" : "text-gray-500 opacity-50")}>
                <Briefcase className="w-6 h-6" /><span className="text-[0.625rem] font-bold uppercase">Remote</span>
            </div>
        </div>
        {systemConfig?.attendance_strict && location === 'OFFICE' && (
            <div className={cn("flex items-center gap-2 px-3 py-1.5 rounded-full text-[0.625rem] font-bold uppercase", distanceFromOffice === null ? "bg-secondary" : distanceFromOffice <= 200 ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500")}>
                ...
                {distanceFromOffice === null ? 'Locating...' : distanceFromOffice <= 200 ? 'Within Range' : 'Outside Range'}
            </div>
        )}
      </div>
    </section>
