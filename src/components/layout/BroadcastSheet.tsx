...
/**
 * Intelligent Broadcast & Messaging System.
...
export function BroadcastSystem({ currentUser, broadcasts = [] }: BroadcastSystemProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({ title: "Broadcast Deleted", description: "The message has been removed from the organizational feed." })
    } catch (e: any) {
        ...
        <SheetHeader className="p-8 pb-4 border-b border-white/5 bg-white/5 shrink-0">
          <div className="flex items-center justify-between mt-2">
            <div className="space-y-1">
                ...
                <SheetDescription className="text-[9px] font-black uppercase tracking-widest opacity-40">Tactical Messaging & Announcements</SheetDescription>
            </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-6 space-y-4 pb-20">
          {broadcasts.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center text-center opacity-20">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em]">Operational feed is currently quiet.</p>
            </div>
          ) : (
            broadcasts.map((msg) => {
                    ...
                    {canModerate && (
                      <DropdownMenu modal={false}>
                        ...
                        <DropdownMenuContent align="end" className="w-48 apple-glass-darker border-white/5 shadow-2xl rounded-xl p-1.5 z-[1000]">
                          ...
                          >
                            <Trash2 className="w-4 h-4 mr-3" /> Purge Post
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    )}
                        ...
                        <AccordionItem value="receipts" className="border-none">
                          <AccordionTrigger className="px-5 py-3 hover:no-underline hover:bg-white/5 transition-all group/receipt">
                             <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-primary group-hover/receipt:text-primary">
                               <Eye className="w-3 h-3" /> {msg.viewedBy?.length || 0} Intelligence Receipts
                             </div>
                          </AccordionTrigger>
                                    ...
                                    return (
                                        <div key={userId} className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/5">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <span className="text-[10px] font-bold text-muted-foreground truncate">{user?.fullName || "Unverified Unit"}</span>
                                            </div>
                                            <span className="text-[8px] font-black uppercase opacity-30">Confirmed</span>
                                        </div>
                                    )
                                })
                             ) : (
                                <p className="text-[8px] font-black uppercase opacity-20 text-center py-2">Waiting for personnel response</p>
                             )}
                          </AccordionContent>
                        </AccordionItem>
                      </Accordion>
