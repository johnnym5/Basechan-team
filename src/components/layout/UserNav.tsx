...
export function UserNav({ userProfile }: { userProfile: UserProfile | null }) {
  ...
  const canAccessAdmin = isSuperAdmin || permissions.canManageCompany;
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-64 apple-glass border-none shadow-3xl mt-2" align="end">
        ...
        {canAccessAdmin && (
          <>
              <DropdownMenuSeparator className="bg-white/5" />
              <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground px-4 py-2">System Control</DropdownMenuLabel>

              {canEnableMode && (
                <div className="flex items-center justify-between p-3 px-4 mb-1">
                    <span className="text-[10px] font-black uppercase tracking-widest text-amber-500">Super Admin Mode</span>
                    <Switch
              ...
              <DropdownMenuItem className="p-3 cursor-pointer text-amber-500 group" onSelect={() => uiEmitter.emit('open-superadmin-dialog')}>
                  ...
                  <span className="font-bold text-xs uppercase tracking-widest">{isSuperAdmin ? 'Master Console' : 'Admin Console'}</span>
              </DropdownMenuItem>

              {isSuperAdminModeActive && (
                <DropdownMenuItem className="p-3 cursor-pointer text-amber-500 group" onSelect={() => uiEmitter.emit('open-database-explorer-dialog')}>
                    <Settings className="mr-3 h-4 w-4 transition-transform group-hover:scale-110" />
                    <span className="font-bold text-xs uppercase tracking-widest">Database Explorer</span>
                </DropdownMenuItem>
              )}
