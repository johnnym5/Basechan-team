    ...
    <DialogPortal>
      ...
      >
        ...
        <div className={cn("mx-auto w-full h-full flex flex-col relative", (finalPosition === "top" || finalPosition === "left") ? "max-w-[1600px]" : "", contentOffsetClass)}>
            <div className={cn("flex-1 min-h-0", isLeftPanel ? "" : "p-6 md:p-8 pt-6")}>
                {children}
            </div>
            ...
            <DialogPrimitive.Close className={cn(
                ...
                isLeftPanel 
                  ...
                  : "right-4 top-4 md:top-6 md:right-6 p-2 bg-secondary text-muted-foreground opacity-70 hover:opacity-100",
                isBottomPanel && "top-0 right-4 translate-y-4 translate-x-0"
            )}>
                {isLeftPanel ? <ChevronLeft className="h-6 w-6" /> : <X className="h-4 w-4 md:h-5 md:w-5" />}
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1 text-center sm:text-left mb-6",
      className
    )}
    {...props}
  />
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-auto pt-6",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "text-2xl md:text-3xl font-black font-headline tracking-tighter leading-tight",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn("text-[10px] md:text-xs font-bold uppercase tracking-widest text-muted-foreground opacity-60", className)}
    {...props}
  />
))
