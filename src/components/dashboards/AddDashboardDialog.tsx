...
interface AddDashboardDialogProps {
  ...
  displayToEdit?: ExternalDisplay | null;
}

export function AddDashboardDialog({ open, onOpenChange, userProfile, displayToEdit }: AddDashboardDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      if (isEditing && displayToEdit) {
        form.reset({
          title: displayToEdit.title,
          url: displayToEdit.url,
  ...
  }, [open, isEditing, displayToEdit, form]);
  ...
  async function onSubmit(values: FormData) {
    ...
    try {
      if (isEditing && displayToEdit) {
        const displayRef = doc(firestore, 'external_displays', displayToEdit.id);
        await updateDoc(displayRef, {
          title: sanitizeInput(values.title),
          url: values.url,
          description: sanitizeInput(values.description),
        });
        await auditService.logAction(firestore, userProfile, 'DISPLAY_UPDATE', `Updated external feed: ${values.title}`, { id: displayToEdit.id, type: 'DISPLAY' });
      ...
      const action = isEditing ? 'Update' : 'Integration';
      ...
      <DialogContent className="sm:max-w-md apple-glass-darker border-none rounded-[2rem]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isEditing ? 'Edit External Feed' : 'Integrate External Feed'}
          </DialogTitle>
          <DialogDescription>
            {isEditing ? 'Update the details for this external feed.' : 'Input the secure embed URL for any dashboard, document, or web tool.'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 py-4">
            ...
            <DialogFooter>
              <Button type="submit" className="w-full h-12 rounded-xl font-black uppercase tracking-widest shadow-xl shadow-primary/20" disabled={isLoading}>
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : (isEditing ? 'Save Changes' : 'Confirm Integration')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
