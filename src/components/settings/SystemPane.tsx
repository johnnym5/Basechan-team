...
import { BrandingSettings } from "./system/BrandingSettings";
import { TemplateSettings } from "./system/TemplateSettings";
import { PolicySettings } from "./system/PolicySettings";
import { TelemetrySettings } from "./system/TelemetrySettings";
...
export function SystemPane({ currentUserProfile, searchTerm }: SystemPaneProps) {
    ...
    const settingsSections = useMemo(() => [
        {
            ...
            description: 'Customize the visual signature of the secure terminal.',
            keywords: ['branding', 'identity', 'color', 'primary', 'accent', 'currency', 'symbol', 'fiscal'],
            component: (
                <Card key="branding" className="apple-glass border-none shadow-xl overflow-hidden">
        ...
        {
            id: 'templates',
            title: 'Finance Reporting Templates',
            description: 'Configure layouts for generated Word/Excel exports.',
            keywords: ['finance', 'reporting', 'templates', 'word', 'excel', 'export', 'banner', 'address', 'legal', 'terms', 'footer', 'document'],
            component: (
                <Card key="templates" className="apple-glass border-none shadow-xl overflow-hidden">
                    <CardHeader className="bg-white/5 border-b border-white/5">
                        <div className="flex items-center gap-3">
        ...
        },
        {
            ...
            description: 'Enable or disable specific modules and compliance behaviors.',
            keywords: ['policies', 'operating', 'modules', 'compliance', 'finance', 'chat', 'comms', 'geofence', 'attendance', 'reporting', 'eod', 'deadline'],
            component: (
                <Card key="policies" className="apple-glass border-none shadow-xl overflow-hidden">
                    <CardHeader className="bg-white/5 border-b border-white/5">
        ...
        {
            id: 'telemetry',
            title: 'Site Telemetry (Coordinates)',
            description: 'Define the office center point for geofencing.',
            keywords: ['site', 'telemetry', 'coordinates', 'location', 'geofence', 'latitude', 'longitude', 'gps', 'device'],
            component: (
                <Card key="telemetry" className="apple-glass border-none shadow-xl overflow-hidden">
                    <CardHeader className="bg-white/5 border-b border-white/5">
    ...
    ], [form, isGettingLocation, handleGetCurrentLocation]);
    ...
    const filteredSections = useMemo(() => {
        ...
        return settingsSections.filter(section =>
            ...
            section.description.toLowerCase().includes(lowercasedTerm) ||
            section.keywords.some(kw => kw.includes(lowercasedTerm))
        );
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12 animate-in fade-in duration-700">
                {filteredSections.length > 0 ? (
                    filteredSections.map(section => section.component)
                ) : (
                    <div className="text-center py-20 opacity-50">
                        <AlertCircle className="mx-auto h-12 w-12 text-muted-foreground" />
                        <h3 className="mt-4 text-lg font-semibold">No Settings Found</h3>
                        <p className="mt-1 text-sm text-muted-foreground">
                            Your search for "{searchTerm}" did not match any settings.
                        </p>
                    </div>
                )}
